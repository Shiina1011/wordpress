	...
	<div class="mgadin-hero">
		<div class="mge-info-content">
			<div class="mge-info-hello">
				...
				$demo_link = esc_url('https://wpthemespace.com/product/x-blog/');
				$pro_link = esc_url('https://wpthemespace.com/product/x-blog/?add-to-cart=146');


				esc_html_e('Hello, ', 'x-blog');
				echo esc_html($current_user->display_name);
				?>

				<?php esc_html_e('👋🏻', 'x-blog'); ?>
			</div>
			<div class="mge-info-desc">
				<div><?php printf(esc_html__('Attention! 🚨 Upgrade to XBlog Pro now and experience the benefits of premade demos and one-click demo import. By purchasing the pro version, you can have a stunning, organized, SEO-friendly, and highly secure WordPress website. Moreover, the pro version enables you to sell anything using WooCommerce. Don\'t wait any longer, upgrade to the Pro version today and enjoy all of its advanced features', 'x-blog'), $current_theme_name); ?></div>
				<div class="mge-offer"><?php printf(esc_html__('Attention free users! Upgrade to XBlog Pro WordPress theme and save 20% with coupon code SPoff20. Transform your website now! ', 'x-blog'), $current_theme_name); ?></div>
			</div>
			<div class="mge-info-actions">
				<a href="<?php echo esc_url($pro_link); ?>" target="_blank" class="button button-primary upgrade-btn">
					<?php esc_html_e('Upgrade Pro', 'x-blog'); ?>
				</a>
				<a href="<?php echo esc_url($demo_link); ?>" target="_blank" class="button button-primary demo-btn">
					<?php esc_html_e('View Details', 'x-blog'); ?>
				</a>
				<button class="button button-info btnend"><?php esc_html_e('Dismiss this notice', 'x-blog') ?></button>
			</div>
...
{
	$hide_date = get_option('xblog_proinfo_hide_date1');
	if (!empty($hide_date)) {
...
function xblog_new_optins_texts_init()
{
	if (isset($_GET['xbnotice']) && $_GET['xbnotice'] == 1) {
		update_option('xblog_proinfo_hide_date1', current_time('mysql'));
	}
}
