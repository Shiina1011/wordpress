		...
		$config = array(
			...
			'getting_started' => array(
				...
				'six' => array(
					...
					'button_url'  => 'http://xm.wpcolors.net/',
					'button_type' => 'link',
					'is_new_tab'  => true,
				),
				'seven' => array(
			...
			'recommended_actions'        => array(
				...
				'content'            => array(
					'magical-posts-display' => array(
						'title'       => __('Magical Posts Display', 'x-magazine'),
						'description' => __('This recommended plugin need to install and active for create awesome posts slider, posts carousel, posts grid, posts list, posts accordion and more.', 'x-magazine'),
...
function x_magazine_admin_notice__error()
{
	...
	$hide_info = get_option('xmag_hide_date');
	if (!empty($hide_info) || $pagenow == 'themes.php') {
		$clickhide = round((time() - strtotime($hide_info)) / 24 / 60 / 60);
		if ($clickhide < 25) {
			return;
		}
	}
	...
	$message = __('<strong>Hi Buddy!! If you want a beautiful, orderly, SEO friendly, more secure and unlimited options website then need to upgrade pro. UPGRADE FOR LIFETIME WITH NOMINAL PRICE. <br><span> GOOD NEWS: X Magazine Pro theme now support woocommerce plugin so now you can sell anything with X Magazine pro theme !!!</span> ', 'x-magazine');
...
function x_magazine_admin_notice_option()
{
	if (isset($_GET['xnotice']) && $_GET['xnotice'] == 1) {
		delete_option('xmagazine8');
		update_option('xmag_hide_date', current_time('mysql'));
	}
}
